...
export const Selection = ({
    ...
    const labelColor = isValueExist ? colors.P_TEXT : colors.S_TEXT
    ...
    const dropArrow = () => {
        return (
            <Icon
                ...
                name={"chevron-down"}
                size={ms(22)}
                style={{ position: 'absolute', right: ws(10), color: colors.P_TEXT }}
            />
        )
    }
    ...
    const placeHolder = () => {
        return (
            <TouchableOpacity onPress={() => setIsPopUpVisible(true)} style={styles.textContainer} activeOpacity={design.OPACITY_AVG}>
                <Text style={[styles.textValue, { color: colors.P_TEXT }]}>{placeholder}</Text>
                {dropArrow()}
            </TouchableOpacity>
        )
    }
    ...
    const renderItem = ({ item, index, isActive, isGrantActive, showArrow, showDelete, key }: any) => {
        const backgroundColor = isActive ? colors.P_TEXT : colors.RED
        const color = isActive ? colors.P_TEXT : colors.P_TEXT
        const onPress = () => {
        ...
        <>
            <Animated.View style={[styles.container,]}>
                ...
                <View>
                    ...
                    {!isEditable ?
                        <View style={{ position: 'absolute', top: 0, left: 0, height: '100%', width: '100%', backgroundColor: colors.TRANSPARENT,  }} />
                        : null}
                </View>
                {error ?
